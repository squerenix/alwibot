...
// TAPI SUBS DappaUhuy NJING JAN CUMA NUMPANG NAMA DOANG BABI!!
// KASI CREDIT DappaUhuy BABI
...
const { color, bgcolor } = require('./dappaganz/color')
const { animesaran } = require('./subsdappauhuy/animesaran')
const { animesaran2 } = require('./subsdappauhuy/animesaran2')
const { help } = require('./dappaganz/help')
const { rules } = require('./subsdappauhuy/rules')
const { iklan } = require('./src/iklan')
const { sewabot } = require('./dappauhuy/sewabot')
const { listsurah } = require('./subsdappauhuy/listsurah')
const { donasi } = require('./dappaganz/donasi')
const { fetchJson } = require('./dappaganz/fetcher')
const { recognize } = require('./dappaganz/ocr')
const { exec } = require("child_process")
const { wait, simih, getBuffer, h2k, generateMessageID, getGroupAdmins, getRandom, banner, start, info, success, close } = require('./dappaganz/functions')
...
numbernye = '0'
fake = 'Muhammad Alwi'
limitawal = '50' // SUKA SUKA LU MAU GANTI APA KAGA
cr = '*JouganBotツ*\n*🍁OW:MhmmdAlwiツ*\n*🍁IG:itsmealwi_*\n*🍁OW:MhmmdAlwiツ*'
vr = '*🍁ANIMESARAN🍁*'
/******** OWNER NUMBER**********/
...
const user = JSON.parse(fs.readFileSync('./dapp/bot/registered.json'))
/*********** END LOAD ***********/
...
fs.existsSync('./session.json') && dappa.loadAuthInfo('./session.json')
dappa.connect();
...
dappa.on('message-update', async (mek) => {
		try {
	   const from = mek.key.remoteJid
		...
		const sender = mek.key.fromMe ? dappa.user.jid : mek.key.remoteJid.endsWith('@g.us') ? mek.participant : mek.key.remoteJid
		...
		if (messageStubType == 'REVOKE') {
			...
			const conts = mek.key.fromMe ? dappa.user.jid : dappa.contacts[sender] || { notify: jid.replace(/@.+/, '') }
			const pushname = mek.key.fromMe ? dappa.user.name : conts.notify || conts.vname || conts.name || '-'
			const opt4tag = {
...
- *Pesan :* *${body ? body : '-'}*`
				dappa.sendMessage(from, strConversation, MessageType.text, selepbot72)
				...
				const savedFilename = await dappa.downloadAndSaveMediaMessage(int.data, `./media/sticker/${filename}`);
				const strConversation = `		 「 *ANTI-DELETE* 」
...
- *Tanggal :* *${moment.unix(int.timestamp).format('DD/MM/YYYY')}*`
				...
				dappa.sendMessage(from, strConversation, MessageType.text, opt4tag)
				dappa.sendMessage(from, buff, MessageType.sticker, pingbro23)
				// console.log(stdout)
				...
				const filename = `${sender.replace('@s.whatsapp.net', '')}-${moment().unix()}`
				const savedFilename = await dappa.downloadAndSaveMediaMessage(int.data, `./media/revoke/${filename}`);
				const buff = fs.readFileSync(savedFilename)
				const strConversation = `	 「 *ANTI-DELETE* 」
...
- *Tanggal :* *${moment.unix(int.timestamp).format('DD/MM/YYYY')}*
- *Pesan :* ${body ? body : '-'}\`\`\``
				dappa.sendMessage(from, buff, MessageType.image, { contextInfo: { mentionedJid: [sender] }, caption: strConversation })
				fs.unlinkSync(savedFilename)
			}
		}
	} catch (e) {
...
// ANTI LINK GRUP
		        ...
		        setTimeout( () => {
			        dappa.groupRemove(from, [kic]).catch((e)=>{reply(`*Jougan HARUS JADI ADMINâ—*`)})
		        }, 3000)
		        setTimeout( () => {
			        dappa.updatePresence(from, Presence.composing)
			        reply("Hedsot....")
		...
		if (!isGroup && !isCmd) console.log(color(time, "white"), color("[ PRIVATE ]", "aqua"), color(budy, "white"), "from", color(sender.split('@')[0], "red"))
			...
			switch(command) {
				...
				if (!isRegistered) return reply(ind.noregis())
						reply(ind.wait())
						fs.writeFileSync('./subsdappauhuy/anime.json', JSON.stringify(anime))
					break
				case 'animesaran':
				if (isBanned) return reply(ind.baned())
					if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
			...
			if (isBanned) return reply(ind.baned())
				dappa.updatePresence(from, Presence.composing) 
 				if (!isRegistered) return reply(ind.noregis())
				 ayam = fs.readFileSync('./subsdappauhuy/music.js');
			...
			if (isBanned) return reply(ind.baned())
				dappa.updatePresence(from, Presence.composing) 
 			if (!isRegistered) return reply(ind.noregis())
				 data = fs.readFileSync('./subsdappauhuy/husbu.js');
		...
		if (isBanned) return reply(ind.baned())
                   ...
                   dappa.sendMessage(from, iklan(prefix), text, { quoted: mek })
                    break
			...
			if (isBanned) return reply(ind.baned())
			 ...
			 reply(`*Sedang di Prosess...*`)
				if (!isRegistered) return reply(ind.noregis())
				 data = fs.readFileSync('./subsdappauhuy/amv.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
		...
		if (isBanned) return reply(ind.baned())
                ...
                await dappa.reply(ind.wait())
                await dappa.sendStickerfromUrl(from, `https://api.vhtear.com/textxgif?text=${q}&apikey=${config.vhtear}`)
                    .then(() => console.log('Success creating GIF!'))
                    .catch(async (err) => {
                        console.error(err)
                        await dappa.reply(from, 'Error!', id)
                    })
            break
                                             case 'woodtext':
			if (isBanned) return reply(ind.baned())
...
//CASE BADWORD
		...
		if (isBanned) return reply(ind.baned())
          ...
          reply('Wait..')
			...
			dappa.sendMessage(from, `${anu.result.log_lengkap}`, text, {quoted: mek})
			await limitAdd(sender) 
			break
                           case 'spamsms':
			if (isBanned) return reply(ind.baned())
          ...
          reply('Wait..')
			...
			dappa.sendMessage(from, `${anu.logs}`, text, {quoted: mek})
			await limitAdd(sender) 
			break
					case 'spamcall':
					case 'call':
		...
		case 'nsfwblowjob':
					...
					if (!isRegistered) return reply(ind.noregis())
						...
						dappa.sendMessage(from, buffervv, image, {quoted: mek, caption: 'Coli teroooooooossss'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
					}
	...
	if (!isRegistered) return reply(ind.noregis())
           ...
           dappa.sendMessage(from, buffer, image, {quoted: mek, caption: '*LOLI IS MY LIFE*'})
		await limitAdd(sender)
           break
		case 'waifu':
		if (isBanned) return reply(ind.baned())
		...
		if (isBanned) return reply(ind.baned())
			...
			break
               ...
               case 'jsholat':
					...
					dappa.sendMessage(from, jsol, text, {quoted: mek})
					break
                case 'sholawat':
                if (isBanned) return reply(ind.baned())        
		    		if (!isRegistered) return reply(ind.noregis())
               ...
               if (anu.error) return reply(anu.error)
                ...
                dappa.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek})
                break
                case 'listonline':
			    ...
			    dappa.sendMessage(from, 'List Online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join`\n`, text, { quoted: mek,
				...
				dappa.updatePresence(from, Presence.composing) 
				 if (!isRegistered) return reply(ind.noregis())
                        if (isLimit(sender)) return reply(ind.limitend(pusname))
				reply(ind.wait())
				 data = fs.readFileSync('./subsdappauhuy/drak.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                 darkjokes = await getBuffer(randKey.result)
                 dappa.sendMessage(from, darkjokes, image, {quoted: mek, caption: '\`\`\`LAMPU MANA LAMPU?\`\`\`'})
				await limitAdd(sender)
				break
			case 'asupan':
				dappa.updatePresence(from, Presence.composing) 
				if (isBanned) return reply(ind.baned())
				if (!isRegistered) return reply(ind.noregis())
				 data = fs.readFileSync('./subsdappauhuy/asupan.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                buffer = await getBuffer(randKey.result)
             ...
             case 'getsticker':
				...
				case 'addsticker':
					...
					delb = await dappa.downloadMediaMessage(boij)
               ...
               if (!isOwner) return reply(ind.ownerb()) 
				...
				case 'getimage':
					...
					dappa.sendMessage(from, buffer, image, { quoted: mek, caption: `Result From Database : ${namastc}.jpeg` })
				...
				case 'listimage':
					...
					dappa.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": imagenye } })
				...
				case 'videolist':
					...
					dappa.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": videonye } })
			...
			if (isBanned) return reply(ind.baned())
					...
					to =  naru[Math.floor(Math.random() * naru.length)];
					nye = await getBuffer(to)
					dappa.sendMessage(from, nye, image, { caption: '*KAKASHI SENSEI*', quoted: mek })
					await limitAdd(sender)
					break 
				case 'minato':
				if (isBanned) return reply(ind.baned())
			...
			if (args.length < 1) return reply(ind.wrongf())
					...
					dappa.sendMessage(from, vintage, image, {caption: 'nih dah jadi ${vin}', quoted: mek})
					await limitAdd(sender)
					break
                 case 'avengers':
                 if (isBanned) return reply(ind.baned())
                 ...
                 if (isLimit(sender)) return reply(ind.limitend(pusname))
                   ...
                   const aruga1 = q.substring(0, q.indexOf('|') - 0)
					...
					dappa.sendMessage(from, aruga, image, {caption: 'Nih su', quoted: mek})
					await limitAdd(sender)
					break 
					case 'summer':
					if (isBanned) return reply(ind.baned())
               ...
               case 'darkneon':
				...
				if (isBanned) return reply(ind.baned())
                    ...
                    dappa.sendMessage(from, buffer, image, {caption: 'Nih kak.. *Jangan lupa follow ig owner @itsmealwi_*', quoted: mek})
                    break
					case 'metaldark':
					if (isBanned) return reply(ind.baned())
					if (!isRegistered) return reply(ind.noregis())
					...
					dappa.sendMessage(from, arugazzz, image, {caption: 'Nih su', quoted: mek})
					await limitAdd(sender)
					break
					case 'lava':
					if (isBanned) return reply(ind.baned())
					...
					dappa.sendMessage(from, shizuka, image, {caption: 'Nih su', quoted: mek})
					await limitAdd(sender)
					break
					case 'blackpink':
					if (isBanned) return reply(ind.baned())
					...
					dappa.sendMessage(from, tigaa1, image, {caption: 'Nih kak', quoted: mek})
					...
					dappa.sendMessage(from, shizukaa, image, {caption: 'Nih su', quoted: mek})
					await limitAdd(sender)
					break
					case 'randomhusbu':
					if (isBanned) return reply(ind.baned())
                ...
                if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                      ...
                      reply(ind.wait())
                      dappa.sendMessage(from, buffere, image, {quoted: mek, caption: 'thund ni '+gh})
                      break
					case 'slide':
					if (isBanned) return reply(ind.baned())
					if (!isRegistered) return reply(ind.noregis())
					...
					dappa.sendMessage(from, bufferooo, image, {quoted: mek})
					break
                    case 'ninjalogo':
                if (isBanned) return reply(ind.baned())
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                      ...
                      dappa.sendMessage(from, bufqf, image, {quoted: mek})
                      break 
					case 'cml':
					case 'ml':
					if (isBanned) return reply(ind.baned())
...
case 'sound':
if (isBanned) return reply(ind.baned())
const soun = fs.readFileSync('./subsdappauhuy/dapuhuy/sound.mp3')
dappa.sendMessage(from, soun, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break 
case 'sound1':
if (isBanned) return reply(ind.baned())
satu = fs.readFileSync('./subsdappauhuy/dapuhuy/sound1.mp3');
dappa.sendMessage(from, satu, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound2':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/sound2.mp3');
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound3':
if (isBanned) return reply(ind.baned())
tiga = fs.readFileSync('./subsdappauhuy/dapuhuy/sound3.mp3');
dappa.sendMessage(from, tiga, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound4':
if (isBanned) return reply(ind.baned())
empat = fs.readFileSync('./subsdappauhuy/dapuhuy/sound4.mp3');
dappa.sendMessage(from, empat, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound5':
if (isBanned) return reply(ind.baned())
lima = fs.readFileSync('./subsdappauhuy/dapuhuy/sound5.mp3');
dappa.sendMessage(from, lima, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound6':
if (isBanned) return reply(ind.baned())
enam = fs.readFileSync('./subsdappauhuy/dapuhuy/sound6.mp3');
dappa.sendMessage(from, enam, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound7':
if (isBanned) return reply(ind.baned())
tujuh = fs.readFileSync('./subsdappauhuy/dapuhuy/sound7.mp3');
dappa.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break 
case 'sound8':
if (isBanned) return reply(ind.baned())
satu = fs.readFileSync('./subsdappauhuy/dapuhuy/sound8.mp3');
dappa.sendMessage(from, satu, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound9':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/sound9.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound10':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/sound10.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound11':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/sound11.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound12':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/sound12.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound13':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/sound13.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound14':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/sound14.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound15':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/sound15.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound16':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/sound16.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'gajelas':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/gajelas.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'iri':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/iri.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'pale':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/pale.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'paplepap':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/PapLepap.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'terpesona':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/terpesona.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'alayy':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/alay.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'bernyanyi':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/bernyanyi.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'bwa':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/bwa.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'ganteng':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/ganteng.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'gatal':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/gatal.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'ladadida':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/ladadida.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'pota':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/pota.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'tb':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/tb.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'tengteng':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/tengteng.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'welot':
if (isBanned) return reply(ind.baned())
dua = fs.readFileSync('./subsdappauhuy/dapuhuy/welot.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'soundquran':
					if (isBanned) return reply(ind.baned())
...
dappa.sendMessage(from, game, image, {quoted: mek})
break
			case 'gemboktext':
			if (isBanned) return reply(ind.baned())
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gem = gh.split("&")[0];
					var bok = gh.split("&")[1];
					if (args.length < 1) return reply(`[❗] Contoh : ${prefix}gemboktext 11 01 2021 & Dappa dan Gaada:v`)
					reply(ind.wait())
		...
		dappa.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})

                break

					case 'kodenegara':

               dappa.sendMessage(from, negara(prefix, sender), text, {quoted: mek})
               ...
               dappa.sendMessage(from, virtex(prefix, sender), text, {quoted: mek})

               break

               case 'murrotal':

		dappa.sendMessage(from, murothal(prefix, sender), text, {quoted: mek})
                ...
                break
					...
					dappa.sendMessage(from, listsurah(prefix) , text, { quoted: mek })
					...
					dappa.sendMessage(from, tnc(prefix) , text, { quoted: mek })
			...
			if (!isRegistered) return reply(ind.noregis())
					...
					dappa.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": user}})
					break
                    case 'xxx':
			    if (isBanned) return reply(ind.baned())
			if (!isRegistered) return reply(ind.noregis())
			...
			if (!isGroup) return reply(ind.groupo())
                ...
                if (!isRegistered) return reply(ind.noregis())
					...
					dappa.sendMessage(from, gools, image, {quoted: mek})
					break
                case 'fast':
     ...
     reply(ind.wait())
            ...
            break
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					dappa.sendMessage(from, nyeee, image, { caption: 'Oni chan baka!! >:(', quoted: mek })
					await limitAdd(sender)
					break
				case 'loli2':
				if (isBanned) return reply(ind.baned())
				...
				if (!isGroup) return reply(ind.groupo()) 
					...
					dappa.sendMessage(from, nyec, image, { caption: 'COLI TERUS!!', quoted: mek })
					break
                  case 'goku':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					dappa.sendMessage(from, nye, image, { caption:'GOKU SUPERSAIYAN!!', quoted: mek })
				...
				if (!isNsfw) return reply(ind.nsfwoff())
				 data = fs.readFileSync('./subsdappauhuy/18.js');
                 ...
                 dappa.sendMessage(from, randBokep, image, {quoted: mek, caption: randTeks})
                 break
				case 'nsfwloli':
				try {   
				if (isBanned) return reply(ind.baned())
				...
				res = await fetchJson(`https://api.vhtear.com/randomloli&apikey=${apivhtear}`, {method: 'get'})
					...
					dappa.sendMessage(from, buffer, image, {quoted: mek, caption: 'nih om'})
				} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
				}
				...
				case 'ero':
				    ...
				    if (isBanned) return reply(ind.baned())
						...
						imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
					dappa.sendMessage(from, buf, image, {quoted: mek,caption: "*HAYO NGAPAIN PEGANG TYTYD?*"})
					})})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('*ERROR*')
            ...
            case 'film':
				...
				dappa.sendMessage(from, film, text, {quoted: mek}) 
					break
			case 'nsfwyuri':
				    try {
				    if (isBanned) return reply(ind.baned())
			...
			case 'nsfwtrap':
					...
					if (!isRegistered) return reply(ind.noregis())
						...
						buffer = await getBuffer(res.result)
						dappa.sendMessage(from, buffer, image, {quoted: mek, caption: '*HAYO NGAPAIN PEGANG TYTYD?*'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('*ERROR*')
					}
			...
			case 'waifu2':
				...
				if (!isNsfw) return reply(ind.nsfwoff())
						...
						buffer = await getBuffer(res.result)
						dappa.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ciee ngaceng'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
			...
			case 'hentai':
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                   ...
                   if (!isNsfw) return reply(ind.nsfwoff())
						...
						buffer = await getBuffer(res.result)
						dappa.sendMessage(from, buffer, image, {quoted: mek, caption: 'hadehh'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
		...
		if (isBanned) return reply(ind.baned())
                   ...
                   dappa.sendMessage(from, buff, image, {quoted: mek, caption: hasil})
                   await limitAdd(sender)
					break 
                   case 'stalkig':
                   case 'igstalk':
		...
		if (isBanned) return reply(ind.baned())
				...
				rest = _limit.indexOf()
				_limit.splice(rest)
				fs.writeFileSync('./dapp/user/limit.json', JSON.stringify(dappaganz))
		...
		ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
			...
			break
               ...
               if (data.error) return reply(data.error)
                ...
                dappa.sendMessage(from, bufferddd, image, {quoted: mek, caption: infomp3})
  ...
  dappa.updatePresence(from, Presence.composing)
	...
	if (!isRegistered) return reply(ind.noregis())
           if (isLimit(sender)) return reply(ind.limitend(pusname))
			...
			reply(ind.wait())
				...
				dappa.sendMessage(from, buff, image, {quoted: mek, caption: 'Lebih baik nulis sendiri ya kak :*'})
				break   	
			case 'pasangan':
			if (isBanned) return reply(ind.baned())
			if (!isRegistered) return reply(ind.noregis())
	  ...
	  if (isBanned) return reply(ind.baned())
		...
		if (isBanned) return reply(ind.baned())
                ...
                reply(ind.wait())
                    ...
                    teks = `${body.slice(9)}`
                    if (teks.length > 10) return dappa.sendMessage(from, 'Teksnya kepanjangan, Maksimal 9 karakter', text, {quoted: mek})
		...
		if (isBanned) return reply(ind.baned())
				...
				if (isLimit(sender)) return reply(limitend(pushname))
					...
					dappa.sendMessage(from, bufferxcz, image, {quoted: mek, caption: 'Dah Jadi Ya Kak'})
					break
		case 'infogempa':
               if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
	...
	if (isBanned) return reply(ind.baned())
		...
		break
				...
				if (!isRegistered) return reply(ind.noregis())
		if (isBanned) return reply(ind.baned())
					dappa.sendMessage(from, donasi(prefix) , text, { quoted: mek })
					break 
                case 'level':
		case 'lvl':
		if (isBanned) return reply(ind.baned())
               ...
               dappa.sendMessage(from, resul, text, { quoted: mek})
				...
				if (!isRegistered) return reply(ind.noregis())
					...
					dappa.sendMessage(from, buffealr, image, {caption: 'Nih kak `${pushname}`', quoted: mek})
					break
                    case 'truth':
                    if (isBanned) return reply(ind.baned())
                if (!isRegistered) return reply(ind.noregis())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
                     const pesan = body.slice(10)
                      if (pesan.length > 300) return dappa.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: mek})
                    ...
                    dappa.sendMessage('6288274362492@s.whatsapp.net', options, text, {quoted: mek})
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
                     const cfrr = body.slice(9)
                      if (cfrr.length > 300) return dappa.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: mek})
                        var nomor = mek.participant
		...
		if (isBanned) return reply(ind.baned())
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					dappa.sendMessage(from, nyev, image, { caption: 'HALLO KANG HALU!!', quoted: mek })
					break
					case 'pictcowok':
					case 'cogans':
				if (isBanned) return reply(ind.baned())
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					dappa.sendMessage(from, nyeb, image, { caption: 'HALLO KANG HALU!!', quoted: mek })
					break
                case 'setmemberlimit':
					if (args.length < 1) return
					if (!isOwner) return reply(ind.ownerb())
      ...
      case 'kodebahasa':
		...
		dappa.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
				...
				if (!isRegistered) return reply(ind.noregis())
					...
					dappa.sendMessage(from, buffergvc, image, {quoted: mek})
					break
		case 'play':
		            if (!isGroup) return reply(ind.groupo())
					if (!isRegistered) return reply(ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
                ...
                case 'textlight':
			    	...
			    	dappa.sendMessage(from, lawak, image, {quoted: mek})
			   	 await limitAdd(sender)
		  	  break
                  case 'groupinfo':
				if (isBanned) return reply(ind.baned()) 
		        ...
		        dappa.sendMessage(from, buffergbl, image, {quoted: mek, caption: `*NAME* : ${groupName}\n*MEMBER* : ${groupMembers.length}\n*ADMIN* : ${groupAdmins.length}\n*DESK* : ${groupDesc}`})
                break
                case 'jadwalTV':
                                case 'jadwaltv':
                                case 'jadwalTv':
                                ...
                                case 'jtv':
                                        ...
                                        teks = `「 *JADWAL TV* 」\n➣ Channel : ${mb} \n➣ *JADWAL* \n${anu.result} `
                                        dappa.sendMessage(from, teks, text)
                                        break
                   case 'kalkulator':    
				   if (isBanned) return reply(ind.baned())
                if (!isRegistered) return reply(ind.noregis())
...
group = await dappa.groupMetadata(from);
...
await dappa.sendMessage(from, options, text)
break
				case 'happymod':
				if (isBanned) return reply(ind.baned())
				if (!isRegistered) return reply(ind.noregis())
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					dappa.sendMessage(from, bufasfer, image, {quoted: mek})
					break
				case 'stiker': 
				case 'sticker':
				case 's':
		...
		if (isBanned) return reply(ind.baned())
			    ...
			    if (!isRegistered) return reply(ind.noregis())
			 	if (isLimit(sender)) return reply(ind.limitend(pusname))
			    if (args.length < 1) return dappa.sendMessage(from, 'Kode bahasanya mana om?', text, {quoted: mek})
					const gtts = require('./dappaganz/gtts')(args[0])
					if (args.length < 2) return dappa.sendMessage(from, 'Textnya mana om', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
					...
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							...
							dappa.sendMessage(from, bufferg, audio, {quoted: mek, ptt:true})
							fs.unlinkSync(rano)
						})
					})
					break
		...
		if (isBanned) return reply(ind.baned())
			...
			if (!isGroup) return reply(ind.groupo())
				...
				if (!isGroup) return reply(ind.groupo())
				if (!isOwner) return reply(ind.ownerb())
				await reply(from, 'bye tugas ku udah selesai☺️').then(() => dappa.groupLeave(from))
					break 
				case 'bc': 
					if (!isOwner) return reply(ind.ownerb()) 
					if (args.length < 1) return reply('.......')
...
case 'snow':
			    ...
			    if (!isRegistered) return reply(ind.noregis())
			 	if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					dappa.sendMessage(from, buffertfd, image, {quoted: mek})
					break						
				case 'add':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
...
case 'bukatime': //by Zitsraa-BOTシ
				...
				case 'promote':
                 ...
                 case 'chatlist':
					...
					teks += `Total : ${totalchat.length}`
					 dappa.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": totalchat}})
					break
				case 'listadmin':
				if (isBanned) return reply(ind.baned())
					if (!isGroup) return reply(ind.groupo())
        	...
        	break
				...
				if (!isQuotedSticker) return reply('Reply/tag sticker !')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						dappa.sendMessage(from, buffer, image, {quoted: mek, caption: 'tuh dh jadi ya kak '})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
				...
				if (argz.length === 1) return dappa.sendMessage(from, `Penggunaan fitur antidelete :\n\n*${prefix}antidelete [aktif/mati]* (Untuk grup)\n*${prefix}antidelete [ctaktif/ctmati]* (untuk semua kontak)\n*${prefix}antidelete banct 628558xxxxxxx* (banlist kontak)`, MessageType.text)
				if (argz[1] == 'aktif') {
					if (isGroup) {
						if (isRevoke) return dappa.sendMessage(from, `Antidelete telah diaktifkan di grup ini sebelumnya!`, MessageType.text)
						dataRevoke.push(from)
						fs.writeFileSync('./src/gc-revoked.json', JSON.stringify(dataRevoke, null, 2))
						dappa.sendMessage(from, `*Succes Enable Antidelete Grup!*`, MessageType.text)
					} else if (!isGroup) {
						dappa.sendMessage(from, `Untuk kontak penggunaan *${prefix}antidelete ctaktif*`, MessageType.text)
					}
				} else if (argz[1] == 'ctaktif') {
					if (!isGroup) {
						if (isCtRevoke) return dappa.sendMessage(from, `Antidelete telah diaktifkan di semua kontak sebelumnya!`, MessageType.text)
						dataCtRevoke.data = true
						fs.writeFileSync('./src/ct-revoked.json', JSON.stringify(dataCtRevoke, null, 2))
						dappa.sendMessage(from, `Antidelete diaktifkan disemua kontak!`, MessageType.text)
					} else if (isGroup) {
						dappa.sendMessage(from, `Untuk grup penggunaan *${prefix}antidelete aktif*`, MessageType.text)
					}
				} else if (argz[1] == 'banct') {
					if (isBanCtRevoke) return dappa.sendMessage(from, `kontak ini telah ada di database banlist!`, MessageType.text)
					if (argz.length === 2 || argz[2].startsWith('0')) return dappa.sendMessage(from, `Masukan nomer diawali dengan 62! contoh 62859289xxxxx`, MessageType.text)
					dataBanCtRevoke.push(argz[2] + '@s.whatsapp.net')
					fs.writeFileSync('./src/ct-revoked-banlist.json', JSON.stringify(dataBanCtRevoke, null, 2))
					dappa.sendMessage(from, `Kontak ${argz[2]} telah dimasukan ke banlist antidelete secara permanen!`, MessageType.text)
				} else if (argz[1] == 'mati') {
					if (isGroup) {
						const index = dataRevoke.indexOf(from)
						dataRevoke.splice(index, 1)
						fs.writeFileSync('./src/gc-revoked.json', JSON.stringify(dataRevoke, null, 2))
						dappa.sendMessage(from, `*Succes disable Antidelete Grup!*`, MessageType.text)
					} else if (!isGroup) {
						dappa.sendMessage(from, `Untuk kontak penggunaan *${prefix}antidelete ctmati*`, MessageType.text)
					}
				} else if (argz[1] == 'ctmati') {
					if (!isGroup) {
						dataCtRevoke.data = false
						fs.writeFileSync('./src/ct-revoked.json', JSON.stringify(dataCtRevoke, null, 2))
						dappa.sendMessage(from, `Antidelete dimatikan disemua kontak!`, MessageType.text)
					} else if (isGroup) {
						dappa.sendMessage(from, `Untuk grup penggunaan *${prefix}antidelete mati*`, MessageType.text)
					}
				}
				break
                			case 'event':
        ...
        if (budy.includes(`ALWI`)) {
                const Dappp = fs.readFileSync('./dappauhuy/Dappp');
        ...
        dua = fs.readFileSync('./subsdappauhuy/dapuhuy/haysayabot.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
...
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
		...
		if (budy.includes(`jougan`)) {
		 ...
		 dua = fs.readFileSync('./subsdappauhuy/dapuhuy/ndabisabasaingris.mp3')
dappa.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
